---
title: LLE
date: 2021-11-25 10:04:33
tags:
---

# Introduction

局部拟合是无监督流形学习方法的核心思想，大多数方法主要考虑增加类间距离和减少类内距离，局部线性嵌入（LLE）在嵌入空间中保留数据的 **局部结构**，从而使得高维空间中的近邻在低维嵌入空间中彼此接近，远点相互远离。本文对 LLE 及其变体进行总结，主要包括以下几个部分：
...
输入空间和特征空间的数据点分别表示为 $\{x_i \in \mathbb{R}^d\}^n_{i=1}$ 和  $\{y_i \in \mathbb{R}^p\}^n_{i=1}$，其中 $p << d$。
...
![QQ截图20211125113423](C:\Users\ZJ\Desktop\QQ截图20211125113423.png)

* **k-Nearest Neighbors**

  构建 k-NN 图。令 $x_{ij} \in \mathbb{R}^d$ 为 $x_i$ 的第 $j$ 个近邻，$\mathbb{R}^{d\times k} \ni X_i := [x_{i1},...,x_{ik}] $ 表示 $x_i$ 的 k 近邻。

* **Linear Reconstruction by the Neighbors**

  利用线性组合的方法，找到重构每个点的线性重构权重，线性重构优化公式为：

  $\min\limits_{\hat{W}} \epsilon(\hat{W}):=\sum\limits_{j=1}^n ||x_i-\sum_{j=1}^k\hat{w}_{ij}x_{ij}||^2_2=\sum\limits_{j=1}^n ||x_i-X_i\hat{w}||^2_2$

  subject to  $\sum\limits_{j=1}^k\hat{w_{ij}}=1, \forall i \in \{1,...,n\}$

  其中 $\hat{w}_i:=[\hat{w}_{i1},...,\hat{w}_{ik}]$ 为 k 近邻线性重构的相应权值。

  线性重构公式可进一步简化为：

  $\min\limits_{\{\hat{w}\}^n_{i=1}} \sum\limits_{i=1}^n\hat{w}_i^TG_i\hat{w}_i$

  subject to  $1^T\hat{w}_i=1, \forall i \in \{1,...,n\}$

  其中 $G_i$ 定义为：

  $\mathbb{R}^{k\times k} \ni G_i :=(x_i1^T-X_i)^T(x_i1^T-X_i)$

  $G_i$ 的秩等于 $min(k,d)$，通常数据是高维的，即 $d << k$，因此 $G_i$ 满秩。 

* **Linear Embedding**

  使用相同的权重，通过嵌入邻居的线性组合嵌入每个点。即

  $\min\limits_{Y}\sum\limits_{i=1}^n ||y_i-\sum\limits_{j=1}^n{w}_{ij}y_j||^2_2$

  subject to  $\frac{1}{n}\sum\limits_{i=1}^ny_iy^T_i=I, \sum\limits_{i=1}^{n}y_i=0$

LLE 的主要思想是在低维嵌入空间中使用和高维空间相同的重构权值。
