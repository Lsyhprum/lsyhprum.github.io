...
本文提出了一种基于 **“遮挡规则”** 构建的理论近邻图及其近似版本，其图结构可以保证获取位于数据集一定距离阈值内查询点的真实最近邻。
...
* **Hashing techniques**
  ...
  * 高召回要求下需要构建多个散列表，内存开销大

  * 不连续散列码间的匹配开销大，不适用于查询点和真实最近邻距离较远的情况

* **Quantisation techniques**

  基于量化的方法能够更好地保留数据在原始空间中的相对距离。然而，该方法和基于 Hash 的方法一样，在高召回率下检索效率迅速下降，当 Recall 接近 1 时，候选列表所需的长度接近数据集的大小。因此，相比于基于 Hash 和量化的方法，连续划分原始数据空间的方法更有效。

* **Tree techniques**

  基于树的方法将数据集连续划分为多个尺度的离散区域。kd-tree 根据数据的外在维度对数据进行聚类，k-means 则尝试根据数据的内在结构对数据进行分组。然而，这些方法在高维数据上表现不佳，当采用错误路径时无法纠正底层的错误，只能重新遍历。

* **Graph techniques**

  基于图的方法能够以类似于树的方式划分搜索空间。典型的 Delaunay 图以确定性的方式进行探索，可以保证找到查询点的真实最近邻。然而，随着数据维数的增加，Delaunay 图将几乎完全连接，计算效率迅速降低；K-NN 图提供了 Delaunay 图中局部邻域的近似值，通过限制图中每个顶点的出度来保持较低的勘探成本。该方法限制牺牲了保证找到真实最近邻的能力，并且数据的一些固有性质（例如密度）会不可避免地丢失。

  对于大型数据集，计算每个顶点的 k 最近邻的计算成本很大。因此，近似 K 近邻图提供了一种替代方法来逼近 Delaunay 图中的边，为图的离线构建提供了显着的加速。然而，这类方法除了对每个顶点的度数进行限制外，图的分散式构建还进一步破坏了 Delaunay  图的理想结构，需要执行回溯和多个同时进行的图探索以实现更高的平均召回率。

# Algorithms

* **Ideal graph structure**

  该论文的关键创新在于通过构建 **单调搜索网络的最小图** 提升搜索效率。单调搜索网络保证无论哪个点作为入口点，总存在一条边通向一个更接近查询点的点，该特性确保了贪婪搜索总能找到正确的最近邻；最小图意味着应尽量去掉冗余的“阻塞”边，只允许保留三角形中较短的边。

  举个栗子，如下图所示，$d(p_1,p_2) < d(p1,p3)$ 且 $d(p_2,p_3) < d(p1,p3)$。根据单调搜索网络性质，总可以从 $p_1$ 通过 $p_2$ 找到 $p_3$，因此，$p_1$ 到 $p_3$ 可以看做冗余的“阻塞”边。

  <img src="occlude.png" alt="occlude" style="zoom:100%;" />
  ...
  > 			
  >　　　　　　if distance($P_u$,$P_v$) < L then　　　　　// 若 $P_u$ 和 $P_v$ 的距离近于 $P_u$ 和 $P_i$ 的距离，则将 $E_j$ 标记为待删除的阻塞边
  > 			
...
* **Making nearest neighbor guarantees**
  ...
  上述算法只能保证通过贪婪算法找到 **数据库内查询点** 的最近邻，为了 **确保数据库外查询也能保持较好的查询效果**，该论文提出了两个改进方法：
  ...
  * **改进图构建算法**
    ...
    <img src="occlude2.png" alt="occlude2" style="zoom:100%;" />
...
* **Efficient graph construction**
  ...
  基础算法时间复杂度很高 O($n^2log(n)$)，论文中通过以下几种方法提高构建速度：
  ...
  * **改进构建算法**
    ...
    * 随机选择两个节点 $v_1$ 和 $v_2$，并通过贪婪搜索尝试从 $v_1$ 到 $v_2$，若未能到达 $v_2$，则 $v_2$ 与访问路径中的最后一个节点添加边。若存在遮挡问题，则需要移除较长的边以维持遮挡规则，并检查搜索路径是否仍然可达。
      ...
      > 										
      >　　E $\leftarrow$ TraverseAdd(P, E, $v_2$, u) 　　　　　　　　　　// test reverse          
      > 		
      > return E
